"""
Import HIQ Rate Contract Excel data into MySQL database
"""
...
def parse_excel_data(excel_path):
    """Parse the HIQ Excel file and extract rate data"""
    ...
    column_specs = {}  # {column_index: {'track_spacing': '...', 'via_drill_pad': '...'}}
    
    # Find track/spacing values (row 19, columns 5+)
    track_spacing_map = {}
    for col_idx in range(4, len(row19)):  # Start from column 5 (index 4)
        if row19[col_idx]:
            track_val = str(row19[col_idx]).strip()
            if 'mil' in track_val.lower():
                track_spacing_map[col_idx + 1] = track_val
    
    # Find via drill/pad values (row 20, columns 5+)
    via_map = {}
    for col_idx in range(4, len(row20)):  # Start from column 5 (index 4)
        if row20[col_idx]:
    ...
    for row_idx in range(22, ws.max_row + 1):
        ...
        if layers_value is not None:
            ...
            if isinstance(layers_value, (int, float)):
                ...
                # Skip rows with text in layers column (like "Additional Cost...")
        ...
        # Skip if thickness contains keywords that indicate it's not a data row
        ...
        for col_idx in range(4, len(row)):  # Start from column 5 (index 4)
            rate_value = row[col_idx]
            
            # Check if it's a numeric rate value
...
def import_to_database(vendor_info, rates_data):
    ...
    try:
        ...
        # Clear existing rates for this vendor (optional - comment out if you want to keep old data)
...
def main():
    ...
    vendor_info, rates_data = parse_excel_data(excel_path)
